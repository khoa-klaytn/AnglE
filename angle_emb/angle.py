...
from transformers import (AutoModelForCausalLM, AutoModel, AutoTokenizer, PreTrainedModel, Trainer,
...
class AngleLoss:
  ...
  def __call__(self,
    ...
    if self.w2 > 0:
      loss += self.w2 * in_batch_negative_loss(
          y_true, y_pred, self.ibn_tau, similar_matrix=similar_matrix)
    if self.w3 > 0:
  ...
  def __call__(self, data: Dict) -> Dict:
    ...
    if self.prompt_template_tok is not None:
      tok1 = self.tokenizer(data['text1'],
                            ...
                            len(self.prompt_template_tok['input_ids']) - extra_length,
                            truncation=True,
    ...
    if self.prompt_template_tok is not None:
      if tok1['input_ids'][-1] != self.prompt_template_tok['input_ids'][-1]:
        print(
            'bad data:',
            f"token ids: {tok1['input_ids']}, prompt token ids: {self.prompt_template_tok['input_ids']}"
        )
        tok1['input_ids'] = self.fix_bad_data(tok1['input_ids'],
                                              self.prompt_template_tok['input_ids'])
        try:
      ...
      if tok2['input_ids'][-1] != self.prompt_template_tok['input_ids'][-1]:
        print(
            'bad data:',
            f"token ids: {tok2['input_ids']}, prompt token ids: {self.prompt_template_tok['input_ids']}"
        )
        tok2['input_ids'] = self.fix_bad_data(tok2['input_ids'],
                                              self.prompt_template_tok['input_ids'])
        try:
...
class Pooler:
  ...
  def __call__(self, inputs) -> Any:
    if self.is_llm:
      hidden_states = self.model(output_hidden_states=True, return_dict=True,
                                 **inputs).hidden_states[-1]
      batch_size = hidden_states.shape[0]
      if self.model.config.pad_token_id is None and batch_size != 1:
        raise ValueError("Cannot handle batch sizes > 1 if no padding token is defined.")
      sequence_lengths = (
          torch.eq(inputs['input_ids'], self.model.config.pad_token_id).long().argmax(-1) - 1).to(
              hidden_states.device)

      outputs = hidden_states[torch.arange(batch_size, device=hidden_states.device),
                              sequence_lengths]
    else:
      ...
      if self.pooling_strategy == 'cls':
        ...
        raise NotImplementedError(
            'please specify pooling_strategy from [`cls`, `last`, `avg`, `max`]')
    return outputs
...
class AnglE:
  ...
  def __init__(self,
    ...
    if is_llm is None:
      ...
      if self.is_llm:
        logger.info(
            'LLM detected, automatically set is_llm=True. If it is wrong, you can manually set `is_llm`.'
        )
    ...
    if self.apply_lora is None:
      if self.is_llm:
        ...
        logger.info(
            'LLM detected, automatically set apply_lora=True. If it is wrong, you can manually set `apply_lora`.'
        )
    if self.device == 'cuda':
    ...
    if self.is_llm:
      logger.info(
          'LLM detected, automatically set prompt. '
    ...
    if self.is_llm:
      ...
      # non-LLMs
      if self.apply_lora:
        model = AutoModel.from_pretrained(pretrained_model_path or model_name_or_path,
                                          trust_remote_code=True)
        if pretrained_lora_path is not None:
          model = PeftModel.from_pretrained(model, pretrained_lora_path, is_trainable=train_mode)
        else:
          peft_config = LoraConfig(**lora_config)
          model = get_peft_model(model, peft_config)
        self.backbone = model
  ...
  def fit(
    ...
    if valid_ds is not None:
      ...
      evaluate_callback = EvaluateCallback(self.backbone,
                                           valid_ds,
    ...
    trainer = AngleTrainer(pooler=self.pooler,
                           model=self.backbone,
                           ...
                           data_collator=AngleDataCollator(
                               ...
                               return_tensors="pt",
                               max_length=self.max_length,
                               compute_similar_matrix=compute_similar_matrix),
                           **trainer_kwargs)
  ...
  def predict(self, data: Dataset, batch_size: int = 32, device: Any = None):
    self.backbone.eval()
    data_collator = AngleDataCollator(self.tokenizer,
                                      return_tensors="pt",
                                      max_length=self.max_length,
  ...
  def set_prompt(self, prompt: str = Prompts.A):
    ...
    if self.prompt is not None:
      logger.info(
          'Prompt is set, the prompt will be automatically applied during the encoding phase. '
  ...
  def encode(self,
    ...
    if self.prompt is not None:
      for i, obj in enumerate(inputs):
        assert isinstance(
            obj, dict), 'The prompt has been set, please pass a dict like {"prompt_key": "text"}'
